...
document
	.getElementById('update-student-link')
	.addEventListener('click', function (event) {
		fetchStudents(event, 'update');
	});
document
	.getElementById('delete-student-link')
	.addEventListener('click', function (event) {
		fetchStudents(event, 'delete');
	});
document
	.getElementById('ban-student-link')
	.addEventListener('click', function (event) {
...
async function deleteStudent(userId) {
	...
	await fetch(`http://localhost:4434/api/Users/${userId}`, {
		method: 'DELETE',
		headers: {
...
async function fetchStudents(event, linkType) {
	...
	await fetch('http://localhost:4434/api/Users/role/students') // Replace '/api/students' with the appropriate API endpoint
...
function populateTableWithData(students, tableBody) {
	...
	students.forEach((student) => {
		...
		const studentIdCell = createTableCell(student.UserId, 'th', {
			scope: 'row',
		});
		...
		const studentEmailCell = createTableCell(
			`${student.UserId}@student.saxion.nl`,
			'td'
		);
...
function populateTableForBanStudent(students, tableBody) {
	...
	students.forEach((student) => {
		...
		const fullNameCell = createTableCell(
		...
		const banStatusCell = createTableCell(
...
async function updateStudent(button) {
	...
	$('#edit-student-modal-title').text(
		`Edit Student: ${chosenStudent.Name} ${chosenStudent.Surname}`
	);
...
async function confirmStudentUpdate(button) {
	if (confirm('Are you sure you want to update this student?')) {
		...
		// Perform the fetch request to delete the student
		await fetch(
			`http://localhost:4434/api/Users/students/${chosenStudent.Id}`,
			{
				method: 'PUT',
				headers: {
					'Content-Type': 'application/json',
