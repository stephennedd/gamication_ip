...
export class GameScene extends Phaser.Scene
{
	...
	constructor() {
      ...
      preload() {
        ...
        this.load.image('player', '../../public/images/ball1.png');
      ...
      create() {
		...
		lifeText = this.add.text(200, 16, 'lives: ' + life, { fontFamily: '"Montserrat"', fontSize: '32px' }).setScrollFactor(0);
		lifeText.depth = 2;
		/* Score Text */
		scoreText = this.add.text(16, 16, 'Score: 0', { fontFamily: '"Montserrat"', fontSize: '32px' }).setScrollFactor(0);
		scoreText.depth = 2;
		/* Game Over Text */
		GameOverText = this.add.text(this.scene.scene.sys.game.canvas.width/2, this.scene.scene.sys.game.canvas.height/2, 'GAME OVER', { fontFamily: '"Montserrat"', fontSize: '90px'}).setScrollFactor(0);
		GameOverText.setOrigin(0.5);
		GameOverText.depth = 2;
		GameOverText.visible = false;
	
		/* Retry Text */
		retryText = this.add.text(this.scene.scene.sys.game.canvas.width/2, this.scene.scene.sys.game.canvas.height/2 + 180, 'RETRY', { fontFamily: '"Montserrat"', fontSize: '32px'}).setScrollFactor(0);
	...
	update() {
		...
		if( player.y > this.cameraYMin + this.scene.scene.sys.game.canvas.height && isProcessing == false ) {
			this.LoseLife();
	...
	/* Create Player Model */
	createPlayer() {
        player = this.physics.add.image(this.scene.scene.sys.canvas.width/2, 3*this.scene.scene.sys.canvas.height/4, "player");
